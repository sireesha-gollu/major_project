...
class Point :
    ...
    def _sub_(self, other) :
        negative = - other
        return self._add_(negative)

    def _mul_(self, scalar: int) :
        return self.curve.mul_point(scalar, self)

    def _rmul_(self, scalar: int) :
...
def heart_beat_measure() :
    ...
    OTP = ""
    for i in range(2) :
        OTP += digits[math.floor(random.random() * 3)]
    return OTP

def decryption(pri_key, C1, C2) :
    cipher_elg = ElGamal(secp256k1)
    return cipher_elg.decrypt(pri_key, C1, C2).decode('utf-8')